...
// Life Goals Options (Matrix 2.2 roles in youth-friendly language)
const LIFE_GOALS_OPTIONS = [
    { id: 'lider_empresarial', text: 'Ser un líder empresarial que toma decisiones estratégicas', icon: '👔' },
    { id: 'creador_negocios', text: 'Crear mi propio negocio y ser mi propio jefe', icon: '💡' },
    { id: 'cientifico_innovador', text: 'Descubrir nuevo conocimiento através de la investigación', icon: '🔬' },
    { id: 'servidor_publico', text: 'Servir a mi comunidad y mejorar políticas públicas', icon: '🏛️' },
    { id: 'educador_inspirador', text: 'Inspirar y enseñar a las nuevas generaciones', icon: '📚' },
    { id: 'profesional_salud', text: 'Cuidar la salud y bienestar de las personas', icon: '🏥' },
    { id: 'agente_cambio', text: 'Generar impacto social positivo y cambiar el mundo', icon: '🌍' },
    { id: 'guardian_planeta', text: 'Proteger el medio ambiente y promover sostenibilidad', icon: '♻️' }
];

// Passions/Competencies Options (in youth-friendly language)
const PASSIONS_OPTIONS = [
    { id: 'cognitivas', text: 'Resolver problemas complejos y pensar críticamente', icon: '🧠' },
    { id: 'sociales', text: 'Conectar con personas y trabajar en equipo', icon: '🤝' },
    { id: 'eticas', text: 'Actuar con integridad y defender lo correcto', icon: '⚖️' },
    { id: 'liderazgo', text: 'Guiar equipos e inspirar a otros', icon: '🎯' },
    { id: 'crisis', text: 'Mantener la calma y actuar bajo presión', icon: '🔥' },
    { id: 'negociacion', text: 'Llegar a acuerdos beneficiosos para todos', icon: '🤝' },
    { id: 'resiliencia', text: 'Levantarme después de los fracasos y seguir adelante', icon: '💪' },
    { id: 'emocional', text: 'Manejar mis emociones y mantener equilibrio', icon: '😊' },
    { id: 'estrategico', text: 'Planificar a largo plazo y anticipar el futuro', icon: '♟️' }
];
...
// Load goals options on page load
function loadGoalsOptions() {
    const lifeGoalsContainer = document.getElementById('lifeGoalsOptions');
    const passionsContainer = document.getElementById('passionsOptions');
    
    if (!lifeGoalsContainer || !passionsContainer) return;
    
    // Life Goals Options
    lifeGoalsContainer.innerHTML = LIFE_GOALS_OPTIONS.map(opt => `
        <div class="col-md-6">
            <label class="goal-option-card p-3 border rounded-3 h-100 d-flex align-items-center" style="cursor: pointer;">
                <input type="checkbox" class="form-check-input me-2 life-goal-checkbox" value="${opt.id}" onchange="handleLifeGoalsSelection()">
                <span><b>${opt.icon}</b> ${opt.text}</span>
            </label>
        </div>
    `).join('');
    
    // Passions Options
    passionsContainer.innerHTML = PASSIONS_OPTIONS.map(opt => `
        <div class="col-md-4">
            <label class="passion-option-card p-2 border rounded-3 h-100 d-flex align-items-center" style="cursor: pointer;">
                <input type="checkbox" class="form-check-input me-2 passion-checkbox" value="${opt.id}" onchange="handlePassionsSelection()">
                <span><b>${opt.icon}</b> ${opt.text}</span>
            </label>
        </div>
    `).join('');
...
function handleLifeGoalsSelection() {
    const checkboxes = document.querySelectorAll('.life-goal-checkbox:checked');
    if (checkboxes.length > 3) {
        alert('⚠️ Máximo 3 opciones permitidas');
        checkboxes[checkboxes.length - 1].checked = false;
        return;
    }
    selectedLifeGoals = Array.from(checkboxes).map(cb => cb.value);
    document.getElementById('lifeGoals').value = selectedLifeGoals.join(', ');
}

function handlePassionsSelection() {
    const checkboxes = document.querySelectorAll('.passion-checkbox:checked');
    if (checkboxes.length > 3) {
        alert('⚠️ Máximo 3 opciones permitidas');
        checkboxes[checkboxes.length - 1].checked = false;
        return;
    }
    selectedPassions = Array.from(checkboxes).map(cb => cb.value);
    document.getElementById('passions').value = selectedPassions.join(', ');
}
...
// Toggle instructor select visibility
function toggleInstructorSelect() {
    const role = document.getElementById('newRole').value;
    const div = document.getElementById('instructorAssignmentDiv');
...
// Edit user - assign instructor
function editUser(userId) {
    const users = DB.getUsers();
    const user = users.find(u => u.id === userId);
    if (!user) return;
    
    const newInstructor = prompt(`Asignar instructor a ${user.fullName} (ID del instructor):`, user.instructorId || '');
    if (newInstructor !== null) {
        user.instructorId = newInstructor || null;
        DB.setUsers(users);
        loadAdminDashboard();
...
function handleAddUser(e) {
    ...
    const users = DB.getUsers();
    const role = document.getElementById('newRole').value;
    const instructorId = document.getElementById('newInstructorId').value;
    ...
    const newUser = {
        ...
        role: role,
        instructorId: role === 'participante' ? instructorId : null
    };
    ...
    document.getElementById('instructorAssignmentDiv').style.display = 'none';
}
