...
// Generate PDF (CORREGIDA - Elemento fuera de pantalla con scroll compensado)
function generatePDF() {
    ...
    // Asegurar que las recomendaciones estén generadas
    if (state.recommendations.length === 0) generateRecommendations();
    
    // Crear contenedor fuera de pantalla (posición negativa)
    const element = document.createElement('div');
    element.id = 'pdfTemporalContainer';
    ...
    // Añadir al body (fuera de la vista)
    ...
    if (state.recommendations && state.recommendations.length > 0) {
        recommendationsHtml = state.recommendations.map(rec => {
            const comp = competencies.find(c => c.id === rec.competency);
            const websHtml = rec.webs?.map(w => `<li style="margin-bottom:5px;"><a href="${w.url}" style="color:#000;text-decoration:underline;">${w.name}</a> - ${w.objective}</li>`).join('') || '<li>No disponible</li>';
            const appsHtml = rec.apps?.map(a => `<li style="margin-bottom:5px;">📱 <a href="${a.url}" style="color:#000;text-decoration:underline;">${a.name}</a> - ${a.objective}</li>`).join('') || '<li>No disponible</li>';
            const videosHtml = rec.videos?.map(v => `<li style="margin-bottom:5px;"><a href="${v.url}" style="color:#000;text-decoration:underline;">${v.name}</a> - ${v.objective}</li>`).join('') || '<li>No disponible</li>';
            const trabajosHtml = rec.trabajos?.map(t => `<li style="margin-bottom:5px;">💼 ${t.name} - ${t.objective}</li>`).join('') || '<li>No disponible</li>';
    ...
    // Añadir sección de recomendaciones y pie
    ...
    // Configuración del PDF con compensación de scroll para capturar el elemento fuera de pantalla
    const opt = {
        margin: 10,
        ...
        html2canvas: { 
            ...
            scrollX: 9999,   // compensa left: -9999px
            scrollY: 9999,   // compensa top: -9999px
            windowWidth: 800,
